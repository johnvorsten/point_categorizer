...
# Read database from .csv file and create an iterator over the file
_master_pts_db = r"D:\Z - Saved SQL Databases\master_pts_db.csv"
my_iter = myClustering.read_database_set(_master_pts_db)

sequence_tag = 'DBPath' # Identifies unique databases from eachother
...
#%%
"""Testing the new NbClust implementation operation complexity"""
...
for _i in range(0,20):
    ...
    # Pre-process raw data and find desired number of clusters
    col = np.where(database.columns == sequence_tag)[0][0]
    db_name = database.iloc[0, col]
    ...
    # Keep track fo dimensionality reduction times
    _t0 = time.time()
    ...
    # Skip databases that are >900 instances (calculation time is too long)
