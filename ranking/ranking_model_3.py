...
model_2 will try training binned labels instead of continuous labels
model_3 (THIS FILE) will train binned labels with embedded per-item features
...
Some things to try : 
1) TODO
(IMPLEMENTED) Embed label space - see tensorflow documentation 
...
_VOCABULARY_FILE = "./data/JV_vocab.txt"
...
_PADDING_LABEL = 0 # For non-reciprocal ranking
_Encoded_labels_dimension = 37 # Number of label features possible (Not used)
_EMBEDDING_DIMENSION = 5 # TODO
...
if __name__ == '__main__':
    ...
    if _confirm:
        ...
        _name = 'Run_' + _now + 'model3'
        _MODEL_DIR = os.path.join(r"TF_Logs\ranking_model_dir", _name)
_NUM_TRAIN_STEPS = 7000


#%% 

def context_feature_columns():
    """Returns context feature names to column definitions. Use this to create
    a feature spec using tf.feature_column.make_parse_example_spec
    """
...
def example_feature_columns():
    """Returns the example feature columns. This does NOT return a TFRecord
    specification. It only returns feature columns which can be used to
...
def input_fn(path, num_epochs=None):
  ...
  #  iterator = tf.data.make_one_shot_iterator(dataset) # Not used
